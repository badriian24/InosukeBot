...
module.exports = {
  ...
  run: async (client, message, args) => {
		const kosong = args[0];
if (!kosong) return message.channel.send({ content: `please input a name for package to search`})
    ...
    if (res.status === 404) return message.channel.send({ content:`No search results found, maybe try searching for something that exists.` });
