...
module.exports = {
            name: "emojiinfo",
            aliases: ["emoteid"],
            usage: "emojiinfo <emoji name, without ::> is no work if emoji deflaut like (😎)",
            category: "info",
        ...
        if (!emoji) return message.channel.send("Could not find emoji... Please provide emoji name or emoji id!, please type emoji name without `<: :>`, if you can see info emoji deflaut like (😎) is not working")
        let xd;
        if(emoji.animated) xd = client.emotes.yes
        if(!emoji.animated) xd = client.emotes.error
